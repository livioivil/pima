...
To model the relationship between the number of deaths and the femininity of the hurricane's name, we adopted a Generalised Linear Model (GLM) with a Quasi-Poisson distribution, since the number of deaths was over-dispersed and right-skewed.  
...
We consider a simple additive regression model like the following one:
...
All four confounding factors have been included in the model in continuous form. In addition to the identity variable, we have decided to explore a series of transformations for each confounder:  
- square root ($\sqrt{x}$).  
- logarithm ($\ln(x)$).  
- second order polynomial: $x+x^2$.   

We want all combinations to contain the four confounders.

```{r,echo=TRUE}
########################################### possible specifications:
poly2<-function(x) poly(x,degree=2)
combinations<-create_multi(~ masfem +min+wind+year+ndam15,focal = "masfem", 
             nfuns = c("log","sqrt","poly2"), 
             data = hurricane)
# keep only full models (those models with four "+" in the formula string)
cmb<-combinations$call[sapply(combinations$calls, str_count, fixed("+"))==4]
```

Then, we evaluate the GLM model for each of the 256 specifications.   

```{r,echo=TRUE}
##################### one list of formulas for each comparison of interest:
#######
formulas1=paste("alldeaths",cmb)
# calculate the model for the first specifications
mod1=glm(as.formula(formulas1[1]),data=hurricane,family=quasipoisson())
# reevaluate the model for all the specifications.
...
```{r,echo=TRUE,set.seed(1)}
res=pima(mods1,tested_coeffs =  c("masfem"),n_flips = 5000,
                          score_type = "standardized",seed = 1)
plot(res,p.values="raw")
...
### Specification curve 
```{r,echo=TRUE}
spec_curve1<-spec_curve(res,alpha = 0.05,p.values = "raw")
spec_curve1
spec_curve2<-spec_curve(res,alpha = 0.05,p.values = "adjusted")
spec_curve2

t_stat_h0<-apply(res$Tspace,1,max)
observed_t<-max(summary(res)$Score)
...
#adjusted pvalues
res$summary_table$p.adj<-jointest:::maxT.light(res$Tspace,exp(seq(-8,0,0.5)))

sum(res$summary_table$p.adj<.05)

library(rpart)
library(rpart.plot)
pima_tree(res) #raw
pima_tree(res,p.values = "adjusted") #FWER adjusted pvalues
```
...
```{r,echo=TRUE}
sel_model<-which.min(res$summary_table$p.adj)
summary(mods1[[sel_model]])
sjPlot::tab_model ( exp(summary(mods1[[sel_model]])$coef[2,1])
```
