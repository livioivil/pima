...
In a simple additive model like this one:

$$g(\mu)=\alpha+\beta_1 mf+ \beta_2 year+\beta_3 min+\beta_4 wind+\beta_5 ndam15$$
The coefficient $\beta_1$ express the increase of the log-death due to a 1-point increase in the $mf$ variable. The results of a Poisson regression models are commonly expressed in terms of Incidence Rate Ratio (IRR) just by exponentiation of the coefficient (IRR=$\exp(\beta_1)$).

### Theoretical model  
We know in advance that the number of deaths depends on the strength of the hurricane (combination of min and wind), if it touches a populated area (ndam15).
...
- 2) The year is a key factor for assessing the spread of information and its use in the general population   
...
All the four confounders are continuous. In addition to the identity, we decide to explore for each variable the following transformations:  
- Square Root ($\sqrt(x)$);  
- Log ($\sqrt(x)$);  
...
Using the pima function() we compute the PIMA procedure  
...
# complexity
pvalue_tree(res=res1,p.values = "adjusted",cp=0.001) #higher complexity
```
